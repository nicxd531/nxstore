  ...
  return (
    ...
    >
        <Box sx={{width:"19%",display:{xs:"none",lg:"flex"},flexDirection:"column",justifyContent:"space-around",height:"100%",alignItems:"start"}}>
            {options.map((data,index)=><Button  onClick={()=>setSelected(index)}  sx={{width:"100%",display:"flex",justifyContent:"start",bgcolor:`${selected == index ?  "#E3F0FF":"transparent"}`,color:`${selected == index ?  "black":"inherit"}`}} key={index}><Link style={{width:"100%"}} href={data.link}>{data.name}</Link> </Button>)}
        </Box>
